/* Crowz-Plugins â€” admin panel logic */
(() => {
    const store = window.CrowzStore;
    if (!store) throw new Error('CrowzStore missing â€” load store.js first.');
    ...
    function renderDashboard() {
        ...
        cards.innerHTML = `
            ...
            <div class="stat-card"><span class="stat-label">Most downloaded</span><span class="stat-value">${top ? top.name : 'â€”'}</span><span class="stat-sub">${top ? fmt(store.downloadCount(top)) + ' downloads' : ''}</span></div>
    ...
    function renderPluginTable() {
        ...
        rows.innerHTML = list.map(p => `
            <tr>
                ...
                <td><div class="p-tag">${p.tag || ''}</div></td>
                <td>v${p.version}</td>
                <td>${p.size}</td>
                <td class="num">${fmt(store.downloadCount(p))}</td>
                <td>${p.updated}</td>
                <td><div class="row-actions">
                    <button class="icon-btn" data-edit="${p.id}" title="Edit">âœŽ</button>
                    <button class="icon-btn danger" data-del="${p.id}" title="Delete">âœ•</button>
                </div></td>
            </tr>`).join('');
    ...
    $('wipe-btn').addEventListener('click', () => {
        confirmDialog('Wipe everything?', 'The catalog, settings and download stats are deleted. The site falls back to its shipped defaults.', () => {
            store.resetCatalog();
            store.resetSettings();
            store.resetStats();
            catalog = store.getCatalog();
