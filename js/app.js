...
(() => {
    ...
    function startMesh() {
        ...
        let W = 0, H = 0;
        const blobs = [
            { x: 0, y: 0, r: 350, color: 'rgba(56, 189, 248, .12)', vx: 0.12, vy: 0.08, phase: 0 },
            { x: 0, y: 0, r: 300, color: 'rgba(167, 139, 250, .08)', vx: -0.1, vy: 0.06, phase: 2 }
        ];
        function resize() {
            ...
            blobs.forEach((b, i) => { b.x = W * (0.3 + i * 0.4); b.y = H * (0.3 + i * 0.2); });
        }
        function step(t) {
    ...
    function startParticles() {
        ...
        function spawn() {
            return { x: Math.random() * W, y: Math.random() * H, vx: (Math.random() - 0.5) * 0.2, vy: -0.1 - Math.random() * 0.15, alpha: 0.08 + Math.random() * 0.15, size: 1 + Math.random() * 1.2 };
        }
        function step() {
            frame++;
            ctx.clearRect(0, 0, W, H);
                ...
                if (frame % 3 === 0) {
                    for (let j = i + 1; j < COUNT; j++) {
                        ...
                        if (d2 < MAX_DIST * MAX_DIST) {
                            ...
                            ctx.moveTo(p.x, p.y);
                            ctx.lineTo(q.x, q.y);
                            ctx.strokeStyle = 'rgba(56, 189, 248, ' + (0.04 * (1 - Math.sqrt(d2) / MAX_DIST)) + ')';
                            ctx.lineWidth = 0.5;
                            ctx.stroke();
                        }
                    }
