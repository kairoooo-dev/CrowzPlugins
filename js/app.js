...
(() => {
    ...
    function createCard(p, index) {
        ...
        card.innerHTML = `
            <a class="card-link" data-open="${p.id}" href="javascript:void(0)">
                <div class="card-icon tile tile-${p.category} tile-lg">${p.icon || p.monogram}</div>
                <div class="card-body">
                    <div class="card-top">
    ...
    function openModal(id) {
        ...
        modalBody.innerHTML = `
            <div class="detail-header">
                <div class="detail-icon tile tile-${p.category} tile-lg">${p.icon || p.monogram}</div>
    ...
    function startAurora() {
        ...
        function resize() { W = canvas.width = window.innerWidth; H = canvas.height = window.innerHeight; }
        const waves = [
            { y: 0.3, amplitude: 60, frequency: 0.003, speed: 0.0008, color1: 'rgba(56, 189, 248, .12)', color2: 'rgba(56, 189, 248, 0)' },
            { y: 0.35, amplitude: 45, frequency: 0.004, speed: 0.0012, color1: 'rgba(167, 139, 250, .10)', color2: 'rgba(167, 139, 250, 0)' },
            { y: 0.4, amplitude: 50, frequency: 0.002, speed: 0.0006, color1: 'rgba(52, 211, 153, .08)', color2: 'rgba(52, 211, 153, 0)' }
        ];
        function step(t) {
            ctx.clearRect(0, 0, W, H);
            waves.forEach(w => {
                ...
                for (let x = 0; x <= W; x += 4) {
                    const baseY = H * w.y;
                    const y = baseY + Math.sin(x * w.frequency + t * w.speed) * w.amplitude
                            + Math.sin(x * w.frequency * 1.5 + t * w.speed * 0.7) * w.amplitude * 0.4;
                    ctx.lineTo(x, y);
                }
                ctx.lineTo(W, H);
                ctx.closePath();
                const grad = ctx.createLinearGradient(0, H * (w.y - 0.3), 0, H * (w.y + 0.4));
    ...
    function startCardTilt() {
        ...
        grid.addEventListener('mousemove', (e) => {
            ...
            if (link) {
                ...
                link.style.setProperty('--mx', lx + '%');
                link.style.setProperty('--my', ly + '%');
            }
        });
    ...
    function renderLicenseList() {
        ...
        licenseListWrap.innerHTML = active.map(l => {
            ...
            return `<div class="license-list-row">
                <div class="license-list-icon tile ${tileClass}">${p ? (p.icon || mono) : mono}</div>
    ...
    function renderLicensePicker() {
        ...
        licensePickerGrid.innerHTML = CATALOG.map(p => {
            ...
            return `<div class="license-pick-card" data-gen-plugin="${p.id}">
                <div class="tile tile-${p.category}">${p.icon || p.monogram}</div>
                <div>
